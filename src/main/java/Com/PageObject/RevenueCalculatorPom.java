...
public class RevenueCalculatorPom extends BaseClass{
	
	@FindBy(how=How.XPATH,using = "//a[@href='/revenue-calculator']//div[contains(text(),'Revenue Calculator')]")
	...
	public void inputTextForSlider() throws InterruptedException, AWTException {
		...
		js.executeScript("arguments[0].scrollIntoView(false);", sliderBall);
	...
	//	WebElement textBoxBySlider = driver.findElement(textBoxBySliderElementDisable);
		...
		js.executeScript("arguments[0].scrollIntoView(false);", textBoxBySlider); // scroll on the element
	...
	public void clickCheckBoxes() throws InterruptedException {
		...
		for (int i = 1; i <= 14; i++) {
			...
			js.executeScript("arguments[0].scrollIntoView(false);", takeTextCPT);
